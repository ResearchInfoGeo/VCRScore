...
class module_VCRScore:
    ...
    def VCRScore (self,ref,caption,file_image):
        model_yolo = yolo.YOLO("YOLO-model.h5")
        _, yolo_classes, _ = model_yolo.detect(file_image)
        
        model_det =detr.DETR()
        detr_classes = model_det.detect(file_image)
        ...
        score_vitl = model_vitl.score_vitl(caption,file_image)
        
        model_clipsc = clipsc.clipsc()
        sc,score_clipsc = model_clipsc.scoreclip(caption,file_image,ref)
        ...
        return value_VCRScore[0],sc,score_clipsc,score_vitl,recall_own,precision_own
